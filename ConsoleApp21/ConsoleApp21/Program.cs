...
namespace ConsoleApp21
{
    internal class Program
    {
        ...
        static public void print_data_in_matrix_format()
        {
            for (int i = 0; i < astronauts.Count; i++)
            {
                Console.WriteLine();
                for(int j = 0; j < astronauts[i].astronaut_list.Count;  j++)
                {
                    if (j+1 == astronauts[i].astronaut_number)
                    {
                        Console.Write($"     {astronauts[i].astronaut_list[j][1]}{String.Concat(Enumerable.Repeat(" ", 3-astronauts[i].astronaut_list[j][1].ToString().Length))}");
                    }
                    else
                    {
                        Console.Write($" {astronauts[i].astronaut_list[j][1]}{String.Concat(Enumerable.Repeat(" ", 3 - astronauts[i].astronaut_list[j][1].ToString().Length))}");
        ...
        static void print_best_comb()
        {
            ...
            {            
                ...
                {
                    Console.WriteLine($"Космонавт номер: {best_combination[i].squad[q].astronaut_number}");
                }
            }
        }
                ...
                else if (key == ConsoleKey.Enter)
                {
                    ...
                    switch (action)
                    {
                        ...
                        case 4:
                            ...
                            Console.WriteLine("Вывод в матричном формате[2](для корректной работы, данные об космонавтах должны быть получены в порядке возрастания их номера)");
